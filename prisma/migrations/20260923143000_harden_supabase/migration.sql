...
-- Addresses Supabase Security/Performance Advisor findings 1-8

-- 1. Fix timestamps: timestamp without tz -> timestamptz (P6). Existing values are UTC instants stored without tz, so interpret as UTC.
...
CREATE POLICY "workspaces_select_member" ON "workspaces" FOR SELECT USING (EXISTS (SELECT 1 FROM "memberships" m WHERE m.workspace_id = "workspaces".id AND m.user_id = auth.uid()));
CREATE POLICY "workspaces_insert_owner" ON "workspaces" FOR INSERT WITH CHECK (auth.uid() = owner_id);
CREATE POLICY "workspaces_update_member" ON "workspaces" FOR UPDATE USING (EXISTS (SELECT 1 FROM "memberships" m WHERE m.workspace_id = "workspaces".id AND m.user_id = auth.uid() AND m.role IN ('owner','admin')));

CREATE POLICY "memberships_select_own" ON "memberships" FOR SELECT USING (user_id = auth.uid() OR EXISTS (SELECT 1 FROM "memberships" m2 WHERE m2.workspace_id = "memberships".workspace_id AND m2.user_id = auth.uid()));
CREATE POLICY "memberships_no_direct_write" ON "memberships" FOR ALL USING (false) WITH CHECK (false);

CREATE POLICY "calendars_select_member" ON "calendars" FOR SELECT USING (EXISTS (SELECT 1 FROM "memberships" m WHERE m.workspace_id = "calendars".workspace_id AND m.user_id = auth.uid()));
CREATE POLICY "calendars_write_owner_admin" ON "calendars" FOR ALL USING (EXISTS (SELECT 1 FROM "memberships" m WHERE m.workspace_id = "calendars".workspace_id AND m.user_id = auth.uid() AND m.role IN ('owner','admin'))) WITH CHECK (EXISTS (SELECT 1 FROM "memberships" m WHERE m.workspace_id = "calendars".workspace_id AND m.user_id = auth.uid() AND m.role IN ('owner','admin')));

CREATE POLICY "events_select_member" ON "events" FOR SELECT USING (EXISTS (SELECT 1 FROM "calendars" c JOIN "memberships" m ON m.workspace_id = c.workspace_id WHERE c.id = "events".calendar_id AND m.user_id = auth.uid()));
CREATE POLICY "events_insert_writer" ON "events" FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM "calendars" c JOIN "memberships" m ON m.workspace_id = c.workspace_id WHERE c.id = calendar_id AND m.user_id = auth.uid() AND m.role IN ('owner','admin','member')));
CREATE POLICY "events_update_writer" ON "events" FOR UPDATE USING (EXISTS (SELECT 1 FROM "calendars" c JOIN "memberships" m ON m.workspace_id = c.workspace_id WHERE c.id = "events".calendar_id AND m.user_id = auth.uid() AND m.role IN ('owner','admin','member')));
CREATE POLICY "events_delete_writer" ON "events" FOR DELETE USING (EXISTS (SELECT 1 FROM "calendars" c JOIN "memberships" m ON m.workspace_id = c.workspace_id WHERE c.id = "events".calendar_id AND m.user_id = auth.uid() AND m.role IN ('owner','admin','member')));

CREATE POLICY "attendees_select_member" ON "attendees" FOR SELECT USING (EXISTS (SELECT 1 FROM "events" e JOIN "calendars" c ON c.id = e.calendar_id JOIN "memberships" m ON m.workspace_id = c.workspace_id WHERE e.id = "attendees".event_id AND m.user_id = auth.uid()));
CREATE POLICY "reminders_select_member" ON "reminders" FOR SELECT USING (EXISTS (SELECT 1 FROM "events" e JOIN "calendars" c ON c.id = e.calendar_id JOIN "memberships" m ON m.workspace_id = c.workspace_id WHERE e.id = "reminders".event_id AND m.user_id = auth.uid()));
CREATE POLICY "recurrence_select_member" ON "recurrence_rules" FOR SELECT USING (EXISTS (SELECT 1 FROM "events" e JOIN "calendars" c ON c.id = e.calendar_id JOIN "memberships" m ON m.workspace_id = c.workspace_id WHERE e.id = "recurrence_rules".event_id AND m.user_id = auth.uid()));

CREATE POLICY "audit_select_member" ON "audit_logs" FOR SELECT USING (EXISTS (SELECT 1 FROM "memberships" m WHERE m.workspace_id = "audit_logs".workspace_id AND m.user_id = auth.uid()));
CREATE POLICY "audit_no_write" ON "audit_logs" FOR ALL USING (false) WITH CHECK (false);

CREATE POLICY "profiles_select_self_or_member" ON "user_profiles" FOR SELECT USING (id = auth.uid() OR EXISTS (SELECT 1 FROM "memberships" m1 JOIN "memberships" m2 ON m1.workspace_id = m2.workspace_id WHERE m1.user_id = auth.uid() AND m2.user_id = "user_profiles".id));
CREATE POLICY "profiles_update_self" ON "user_profiles" FOR UPDATE USING (id = auth.uid()) WITH CHECK (id = auth.uid());
...
-- Re-grant minimal needed for authenticated to even hit RLS (no-op if fully Prisma-only, but keep SELECT for future if Data API re-enabled)
-- For now keep no grants — Prisma uses service_role/postgres which bypasses RLS and does not need these.
-- If you later need Data API, re-add: GRANT SELECT ON ... TO authenticated;
